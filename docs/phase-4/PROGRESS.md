# Phase 4 — progress log

Claude Code / Cursor: this folder is the Phase 4 working set.
Plan: `docs/phase-4-plan.md`. Assets: `brand/`, `public/brand/`, `public/sounds/`.

## Status

- [x] Branch `phase-4/brand-landing`
- [x] Plan + decisions 47–51
- [x] Logo SVG + raster + favicon + AppIcon.appiconset
- [x] Sound kit WAV (ffmpeg absent on this machine → no MP3; play path is WAV)
- [x] `src/lib/sounds.ts` + Settings toggle + preview
- [x] 8-section landing + legal views
- [x] i18n 5 langs + missing-key test
- [x] Gate: typecheck, lint, vitest (401), build
- [x] Centered Anthropic/Linear hero (geometry + grid removed)
- [x] Exam marquee under fold (13 boards, 40s, pause on hover, edge fade)
- [x] SVG orbs around predictor (not emoji)
- [x] CTA hover micro-timeline (Day 1 / 3 / 7 / 47)
- [x] Split-scroll: type left, Learn mock center, predictor right — CSS, no Framer Motion
- [x] Product theater + “beta has no reviews” removed
- [x] Horizontal feature reel: chat → Learn → geometry → calendar
- [x] About copy is the $4 level-up pitch + CTAs on every block
- [x] Orbit rings + upright discs (not hidden on mobile)
