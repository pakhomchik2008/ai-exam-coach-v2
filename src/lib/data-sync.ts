...
/**
 * Patches `localStorage.setItem`/`removeItem` once, at the single point every
 * store's writes already pass through — this is what lets the sync layer see
 * every store's writes without editing eight store files individually.
 *
 ...
 */
function patchLocalStorage(sb: SupabaseLike, userId: string, keys: readonly string[]): void {
  if (_patched) return;
  _patched = true;
  ...
  localStorage.setItem = (key: string, value: string) => {
    ...
    if (!_writingFromSync && keySet.has(key)) schedulePush(sb, userId, key);
  };

  const originalRemoveItem = localStorage.removeItem.bind(localStorage);
  localStorage.removeItem = (key: string) => {
    originalRemoveItem(key);
    // A tracked key being removed outside the sync layer's own writes only
    // happens today via clearSession()'s logout loop, which also tears this
    // subscription down (see stopDataSync below) — nothing to push for a key
...
export async function startDataSync(sb: SupabaseLike, userId: string): Promise<void> {
  ...
  const onVisible = () => {
    if (document.visibilityState === "visible") void runReconcile(sb, userId, keys);
  };
