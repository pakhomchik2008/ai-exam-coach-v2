...
- **Health Score**: **5.5 / 10**
  ...
  - **−** Identity model (`examId::topicIdx`) is positional and used in 108 places; one AI-generated topic list in a different order silently rewrites a student's history. Zero automated test coverage outside one manual calendar suite. ~~`api/complete.js` is an unauthenticated open proxy onto a paid Anthropic key.~~ (fixed — see S1) A stale 384 KB `_ds_bundle.js` shadow-defines 25 of the app's own component globals.
- **Top 3 Risks**:
  ...
  2. ~~**Open AI proxy**~~ — **FIXED.** `/api/complete` and `/api/fetch-url` now require a Supabase JWT, enforce an Origin allowlist, cap payload size, and spend from a per-user daily quota (`supabase/07_ai_usage.sql`). See S1.
...
|---|---|---|---|---|
| ~~S1~~ | ~~**Unauthenticated AI proxy.**~~ **FIXED.** `api/_guard.js` now gates both serverless functions: Origin allowlist → Supabase JWT verified against `/auth/v1/user` → per-user/per-UTC-day quota in `public.ai_usage` (`supabase/07_ai_usage.sql`). `api/complete.js` also caps payloads at 200 000 chars / 80 messages and reports real token spend back to the quota. "Try the demo" signs in anonymously so it keeps working on a smaller budget (`complete:anon`). | `api/_guard.js`, `api/complete.js`, `api/fetch-url.js`, `supabase/07_ai_usage.sql`, `auth-store.jsx`, `index.html` | ~~Critical~~ Closed | — |
| ~~S2~~ | ~~**Community curriculum rows override the official seed for all users.**~~ **FIXED.** Client merge is now trust-ranked (`TRUST` tiers in `curriculum-store.jsx`): a contribution can fill a gap but never displace curated content, the board-specific override applies only within the top tier, contributed rows carry no aliases, and the section-union path excludes untrusted rows outright. Server side (`supabase/08_curriculum_trust.sql`) forces `source`/`moderation_status`, blocks anonymous writers, refuses any subject that already has a curated row, caps size, attributes and rate-limits. Contributions are published only after a human confirms them. 28 regression tests in `scripts/curriculum-trust-tests.mjs`. | `curriculum-store.jsx`, `supabase/08_curriculum_trust.sql`, `CurriculumStep.jsx` | ~~High~~ Closed | — |
...
|---|---|---|---|
| ~~**1. Authenticate `/api/complete`**~~ — **DONE.** JWT + per-user daily budget + Origin allowlist + payload caps, applied to `/api/fetch-url` too. Two deploy prerequisites remain: run `supabase/07_ai_usage.sql`, set `SUPABASE_SERVICE_ROLE_KEY` on Vercel, and enable Supabase anonymous sign-ins (for demo mode). | `api/_guard.js`, `api/complete.js`, `api/fetch-url.js`, `supabase/07_ai_usage.sql`, `auth-store.jsx`, `index.html`, `Landing.jsx` | done | R2. Unbounded, ongoing financial exposure on a live URL. Nothing else matters if the key dies. |
